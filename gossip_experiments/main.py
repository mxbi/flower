...
parser = argparse.ArgumentParser(description="Flower Simulation with PyTorch")
        ...
        # gossip_count=args.gossip_count,
        # gossip_segments=args.gossip_segments,
        # pga_frequency=args.pga_frequency,
parser.add_argument("--num_client_cpus", type=int, default=1)
parser.add_argument("--num_rounds", type=int, default=100)

parser.add_argument("--gossip_count", type=int, default=1)
parser.add_argument("--gossip_segments", type=int, default=1)
parser.add_argument("--pga_frequency", type=int, default=0)
...
def evaluate_metrics_aggregation_fn(metrics):
    count = sum(m[0] for m in metrics)
    d = {}
    for key in metrics[0][1].keys():
        d[key] = sum(m[1][key] * m[0] for m in metrics) / count
    return d
...
if __name__ == "__main__":
    ...
    # )
    strategy = fl.server.strategy.GossipAvg(
        gossip_count=args.gossip_count,
        gossip_segments=args.gossip_segments,
        pga_frequency=args.pga_frequency,
        fraction_fit=0.1,
    ...
    pickle.dump(results, open(f"results-{args.gossip_count}-{args.gossip_segments}-{args.pga_frequency}.pkl", "wb"))
